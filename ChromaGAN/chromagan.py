...
class generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.vgg_model = nn.Sequential(*[self.vgg_model._features[i] for i in range(30)])
        # Output Dimensions 28 x 28 x 512

        ## Global Features
        self.global_features = nn.Sequential(
            nn.Conv2d(512,512, kernel_size=3, padding = 1, stride = 2, bias=False),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(512),

            nn.Conv2d(512,512, kernel_size=3, padding = 2, stride = 1, bias=False),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(512),
    ...
    # After repetition and resahping -> Output Dimension 28 x 28 x 256
    ...
    self.midlevel_features  = nn.Sequential(
        ...
        nn.BatchNorm2d(512),
         ...
         nn.Conv2d(512,256,kernel_size=3, stride=1, padding = 2, bias = False),
        nn.ReLU(inplace = True),
        nn.BatchNorm2d(512)
    )
